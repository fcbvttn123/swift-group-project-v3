        ...
        profilesCollection.document(currentUserUID).setData(["HomeCampus": mapItemString], merge: true) { error in
            if let error = error {
                ...
                if let homeLocation = mapItem.placemark.location {
                    let coordinatesString = self.locationToString(location: homeLocation)
                    
                    // Add the home campus coordinates to HomeCampusCoordinates
                    let homeCoordinates = [
                        "latitude": homeLocation.coordinate.latitude,
                        "longitude": homeLocation.coordinate.longitude
                    ]
                    profilesCollection.document(currentUserUID).setData(["HomeCampusCoordinates": homeCoordinates], merge: true) { error in
                        if let error = error {
                            print("Error updating HomeCampusCoordinates: \(error)")
                        } else {
                            print("HomeCampusCoordinates updated successfully")
                        }
                    }
                    
                    profilesCollection.document(currentUserUID).updateData([
                        "AvailableCampuses": FieldValue.arrayUnion([coordinatesString])
                    ]) { error in
                        if let error = error {
                            print("Error updating AvailableCampuses: \(error)")
                        } else {
                            print("AvailableCampuses updated successfully")
